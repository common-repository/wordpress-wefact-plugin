...
class WPWF_Plugin {
	...
	public function wpwf_debtors() {
		...
		if ( empty($listDebtors['Error'])) {
			...
			$curpage 		= (empty($_GET['pagination']) ? 1 : $_GET['pagination']);
	...
	public function wpwf_invoices() {
		...
		$count = $this->wpwf_count($listInvoices['result']);

		if (empty($listInvoices['error'])) {
			$curpage 		= (empty($_GET['pagination']) ? 1 : $_GET['pagination']);
			$pages 			= ceil($count / $this->limit);
			$start 			= ($curpage - 1) * $this->limit;
	...
	public function wpwf_count($result) {
		$count = 0;
	...
	public function wpwf_pricequotes() {
		...
		if (empty($listPriceQuotes['error'])) {
	...
	public function wpwf_products() {
		...
		$count = $this->wpwf_count($listProducts['result']);

		if (empty($listProducts['error'])) {
			$curpage 		= (empty($_GET['p']) ? 1 : $_GET['p']);
			$pages 			= ceil($count / $this->limit);
			$start 			= ($curpage - 1) * $this->limit;
